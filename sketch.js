...
function preload() {
    ...
    paperImg = loadImage(
        "https://cors-anywhere.herokuapp.com/https://raw.githubusercontent.com/Anay121/InfiniteVenues/master/static/VTGPaper.png"
    );
    scissorImg = loadImage(
        "https://cors-anywhere.herokuapp.com/https://raw.githubusercontent.com/Anay121/InfiniteVenues/master/static/VTGScissors.png"
    );
    heartImg = loadImage(
        "https://cors-anywhere.herokuapp.com/https://raw.githubusercontent.com/Anay121/InfiniteVenues/master/static/lives.png"
    );
    playerImg = loadImage(
...
//     } else {
//         mytimeout = setTimeout(spawner, time);
//     }
// });
    ...
    Events.on(engine, "collisionStart", function (event) {
        ...
        if (pairs[0].bodyA.label == "rock" && pairs[0].bodyB.label == "paper") {
            ...
            incomingStuff = incomingStuff.filter((elem) => {
                return (
                    elem.body.id != pairs[0].bodyA.id && elem.body.id != pairs[0].bodyB.id
                );
            });
        }
        ...
        if (
            pairs[0].bodyA.label == "scissors" &&
            pairs[0].bodyB.label == "scissors"
        ) {
            World.remove(world, pairs[0].bodyB);
            incomingStuff = incomingStuff.filter((elem) => {
                return elem.body.id != pairs[0].bodyB.id;
...
function specialPower() {
    ...
    elems.forEach((elem) => {
        let sy = -7;
        let sx =
            (-(7 + elem.body.velocity.y) * (playerPos.x - elem.body.position.x)) /
            (playerPos.y - elem.body.position.y);
